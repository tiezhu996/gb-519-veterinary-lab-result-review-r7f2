...
type AnimalCaseHandler struct{ service service.AnimalCaseService }

func NewAnimalCaseHandler(s service.AnimalCaseService) *AnimalCaseHandler {
	return &AnimalCaseHandler{service: s}
}

func (h *AnimalCaseHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/cases")
	resource.GET("", h.list)
...
func (h *AnimalCaseHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *AnimalCaseHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *AnimalCaseHandler) create(c *gin.Context) {
	var input dto.CreateAnimalCase
...
func (h *AnimalCaseHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateAnimalCase
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *AnimalCaseHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *AnimalCaseHandler) remove(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
