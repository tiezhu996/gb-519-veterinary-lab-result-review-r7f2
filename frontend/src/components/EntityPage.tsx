...
import { CriticalGatePanel } from './common/CriticalGatePanel';
...
interface EntityPageProps {
  ...
  gateVariant?: 'assay' | 'signoff';
}

export function EntityPage({ config, useStore, showRiskTags = false, showResultPanel = false, gateVariant }: EntityPageProps) {
  ...
  // 危急检验结果处置闸门：关联编号存在未放行事项（待处置/已失效）时阻断提交复核。
  const gateBlocksReview = (item: DomainRecord) => Boolean(
    requiresPreparer(item) && item.gate && item.gate.status !== 'confirmed',
  );
  const canAdvance = (item: DomainRecord) => canOperate
    ...
    && !gateBlocksReview(item);

  const unavailableReason = (item: DomainRecord) => {
    if (!canOperate) return '只读';
    if (gateBlocksReview(item)) return item.gate?.status === 'void' ? '运行失效·处置已作废' : '危急结果待处置';
  ...
  const showGate = Boolean(gateVariant);
  const gateCount = useMemo(() => new Set(items.flatMap((item) => {
    if (gateVariant === 'assay') return (item.dispositions || []).map((entry) => entry.relatedCode);
    return item.gate ? [item.gate.relatedCode] : [];
  })).size, [items, gateVariant]);
  const gateVariantValue = gateVariant ?? 'assay';
  ...
  return <main className="workspace">
    ...
    <section className="metrics"><MetricCard label="记录总数" value={meta.total} detail="当前筛选范围"/><MetricCard label="高风险" value={highRisk} detail="需要优先复核"/><MetricCard label="危急处置事项" value={gateCount} detail="处置闸门关联编号"/></section>
    {showGate && <section className="result-section gate-section"><header><h2>危急检验结果处置闸门</h2><span>核验通过的严重风险运行必须先确认处置，关联结果方可复核</span></header><CriticalGatePanel records={items} variant={gateVariantValue} onChanged={() => void load(config.path, search)} /></section>}
    ...
    <section className="table-shell" aria-busy={loading}><table><thead><tr><th>编码</th><th>名称</th><th>状态</th><th>风险</th><th>责任人</th><th>指标</th>{showGate && <th>处置闸门</th>}<th>更新时间</th><th>操作</th></tr></thead><tbody>
      {items.map((item) => { const next = nextStatus(item.status, config.primaryTransitions); const latestAssayDisposition = item.dispositions?.[0]; return <tr key={item.id}><td><strong>{item.code}</strong></td><td>{item.name}<small>{item.facility}</small></td><td><StatusBadge status={item.status}/></td><td>{showRiskTags ? <RiskTag level={item.riskLevel}/> : item.riskLevel}</td><td>{item.owner}<small>{config.key === 'assayRun' && item.operatedBy ? `操作员 ${item.operatedBy}` : null}</small></td><td>{item.metricValue} {item.metricUnit}</td>{showGate && <td>{gateVariant === 'signoff' ? (item.gate ? <DispositionBadge status={item.gate.status} /> : <span className="muted">普通结果</span>) : (latestAssayDisposition ? <DispositionBadge status={latestAssayDisposition.status} /> : <span className="muted">-</span>)}</td>}<td>{formatDate(item.updatedAt)}</td><td>{next && canAdvance(item) ? <button className="table-action" onClick={() => setPending({ item, status: next })}>推进至 {next}</button> : <span className="muted">{next ? unavailableReason(item) : '流程结束'}</span>}</td></tr>; })}
      {!items.length && !loading && <EmptyState message="暂无记录" colSpan={showGate ? 9 : 8} />}
